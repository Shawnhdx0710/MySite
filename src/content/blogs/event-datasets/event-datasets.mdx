...
| [SPORTEC](#sportec) | XML | Direct download | CC-BY 4.0 | 7 matches (tracking + events) |
| [WhoScored](#whoscored) | JSON (in page) | Scrape | Proprietary, ToS-restricted | ~1.4k per match |
| [PFF FC](#pff) | JSON / JSONL | Form request | Free for research | 64 WC games (tracking + events) |
...
[IMPECT](https://github.com/ImpectAPI/open-data) is a German football analytics provider. It released open-access event data for the Bundesliga 2023/24 season. The data includes the event stream plus IMPECT's own performance KPIs, such as bypassed opponents. Those KPIs are unique to IMPECT and do not appear in the other datasets here.

### Coverage

The dataset covers the Bundesliga 2023/24 season. It includes match events, event-level KPIs, player KPIs, lineups and substitutions, match info, and player info.

### Structure

The data ships as JSON in a GitHub repository. Separate folders hold events, event KPIs, and player KPIs. File names contain the match ID.
...
The easiest path is [kloppy](https://github.com/PySport/kloppy), which loads IMPECT's open data directly (kloppy 3.18 or later):
...
The dataset covers seven matches from the German Bundesliga 2022/23 season. Two are from the first division, and five are from the second division. It covers 207 players, 11,137 events, and over one million frames of x/y positions for players and the ball.

### Structure

Each match has three XML files: match information, match events, and position data. The position data is TRACAB tracking at 25 Hz. All definitions follow the DFL catalogue.

### Loading

Download the files from [figshare](https://doi.org/10.6084/m9.figshare.28196177) or load them with the [floodlight](https://github.com/floodlight-ai/floodlight) package.
...
[PFF FC](https://www.blog.fc.pff.com/blog/pff-fc-release-2022-world-cup-data) released a dataset for the 2022 World Cup. Pro Football Focus started as an American football analytics company, and PFF FC is its soccer arm. The dataset combines broadcast tracking data, event data, and PFF's play-by-play player grades for all 64 matches.

### Coverage

The dataset covers all 64 matches of the 2022 World Cup. It pairs broadcast tracking data with event data and a play-by-play grade for every player.

### Structure

Tracking files are stored per game as `{game_id}.jsonl.bz2`. Event data sits in a per-game "Event Data" folder. Metadata files hold players, rosters, and competitions. The grades run from -2 to +2 in 0.5 steps and convert to a 0-100 scale at the season level.
